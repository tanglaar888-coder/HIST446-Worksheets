...
>the third and first vector are omitted.
...
>108 checks the values against the true value and lists the true one.
...
>assigns names to vectors and lists vector when given name
...
>202, the comma expanded how many columns displayed
...
>the gayguides$title is the colmun and to search the rows need to have a [] thus search the rows for the state
```{r R.options=list(max.print=10)}
...
Hint: Break this into steps:
   ...
   > how to find the year in gayguide
